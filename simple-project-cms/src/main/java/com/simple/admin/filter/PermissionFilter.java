package com.simple.admin.filter;
  ...
  implements Filter
{
    ...
    throws IOException, ServletException
  {
    ...
    if (isExclude(currURI)) {
      ...
      LoginUser user = LoginUserUtil.getCurrentUser(request);
      if ( null == user ) {
...
//		}
//	} else {
		AjaxWebUtil.sendAjaxResponse(request, response,new ResponseInfo(new ResponseStatus(false,"3","当前登录信息失效，请退出并重新登录!"), null));
//	}
  }
}